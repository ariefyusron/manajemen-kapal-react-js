...
const Component = () => {
  ...
  const _renderTable = useCallback(
    ...
    [dispatch, standarTarifState.list]
  );
  ...
  return (
    <Container isLoading={standarTarifState.isLoading}>
      ...
      {standarTarifState.listPekerjaan.length > 0 && (
        <Row className="align-items-center" style={{ marginBottom: 20 }}>
          <Col size={10}>
            <Row>
              <Col size={6}>
                <select
      ...
      {selectPekerjaan === ""
        ? standarTarifState.listPekerjaan.map((e, i) => _renderTable(e, i))
        : standarTarifState.listPekerjaan.map(
            (e, i) =>
